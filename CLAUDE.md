...
9. **"Customer" has one definition, everywhere:** a pass that was stamped, is
...
10. **No browser dialogs for anything that matters.** Browsers let a user
...
6. **Auth is hand-rolled on node:crypto** (scrypt + HMAC cookies, timing-safe
   ...
   owner session, per-café enrollment (dedup), and **per-owner** staff session.
   **There is ONE staff PIN per owner** (`owners.staff_pin_hash`), covering
   ...
   hang off each café row, so "+ Add card" silently minted a second PIN and a
   second stamper link; don't reintroduce that. Which card a staff request is
   about travels in `x-cafe-id` and is checked against that owner's cards.
